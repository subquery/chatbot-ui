...
const Home = ({
  ...
  const handleNewConversation = async () => {
    ...
    if (models && models.length > 0) {
      ...
      const updatedConversations = cloneDeep([
        ...conversations,
        newConversation,
      ]);
  ...
  const handleUpdateConversation = async (
    ...
    const { single, all } = await updateConversation(
      updatedConversation,
      conversations,
    );
  ...
  const handleUpdateConversationAll = async (
    conversation: Conversation,
    data: KeyValuePair[],
  ) => {
  ...
  const recoveryConversation = async () => {
    const conversationHistory = await localforage.getItem<string>(
      'conversationHistory',
    );
    ...
    const selectedConversation = await localforage.getItem<string>(
      'selectedConversation',
    );
  ...
  return (
    ...
    >
      <Head>
        ...
        />
        <link rel="icon" href="https://static.subquery.network/icon.png" />
      </Head>
      {selectedConversation && (
        <main
